...
class ZKTecoCloudConfig(models.Model):
    ...
    def _process_attendance_data(self, attendance_data):
        ...
        # Pre-cache devices and user mappings to avoid N+1 queries
        incoming_device_ids = list({str(r.get(self.device_id_field, r.get('device_id'))) for r in attendance_data if r.get(self.device_id_field, r.get('device_id'))})
        incoming_user_ids = list({str(r.get(self.user_id_field, r.get('user_id'))) for r in attendance_data if r.get(self.user_id_field, r.get('user_id'))})
        
        devices = self.env['zkteco.device'].search([('serial_number', 'in', incoming_device_ids)])
        device_cache = {d.serial_number: d for d in devices}
        
        mappings = self.env['zkteco.user.mapping'].search([('device_user_id', 'in', incoming_user_ids)])
        mapping_cache = {m.device_user_id: m for m in mappings}
        
        employee_ids = [m.employee_id.id for m in mappings if m.employee_id]
        existing_attendances = self.env['zkteco.attendance'].search([('employee_id', 'in', employee_ids)])
        attendance_set = {(a.employee_id.id, a.timestamp, a.device_id.id) for a in existing_attendances}
        ...
        for record in attendance_data:
            try:
                ...
                device_id_raw = record.get(self.device_id_field, record.get('device_id'))
                user_id_raw = record.get(self.user_id_field, record.get('user_id'))
                ...
                if not all([user_id_raw, timestamp_str]):
                    _logger.warning(f"Incomplete record: {record}")
                    continue
                    
                device_id_str = str(device_id_raw)
                ...
                device = device_cache.get(device_id_str)
                if not device:
                    device = self.env['zkteco.device'].create({
                        'name': f'Cloud Device {device_id_str}',
                        'ip_address': 'cloud',
                        'serial_number': device_id_str,
                        'is_online': True,
                    })
                    device_cache[device_id_str] = device
                
                # Find employee mapping
                user_mapping = mapping_cache.get(user_id_str)
                ...
                if not user_mapping:
                    quarantine_vals.append({
                        ...
                        'error_reason': f'No employee mapping found for user ID: {user_id_str}',
                        'raw_data': json.dumps(record),
                    })
                    continue
                
                # Check for duplicate
                if (user_mapping.employee_id.id, timestamp, device.id) in attendance_set:
                ...
                attendance_set.add((user_mapping.employee_id.id, timestamp, device.id))
                processed_count += 1
        ...
        if attendance_vals:
            self.env['zkteco.attendance'].create(attendance_vals)
