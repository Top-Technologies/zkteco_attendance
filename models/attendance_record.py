...
class AttendanceRecord(models.Model):
    ...
    leave_period = fields.Selection([
        ...
        ('am', 'Morning (AM)'),
        ('pm', 'Afternoon (PM)'),
    ], string='Leave Period', compute='_compute_leave_info', store=True, readonly=False)
    ...
    @api.depends('status', 'missed_punch_type', 'leave_id', 'leave_type_id', 'leave_type_id.name', 'is_half_day_leave', 'leave_period', 'is_paid_leave')
    def _compute_status_display(self):
        for record in self:
            if record.status == 'missed_punch':
                ...
                type_name = record.leave_type_id.name if record.leave_type_id else "Time Off"
                if record.is_half_day_leave and record.leave_period:
                    period_str = "AM" if record.leave_period == 'am' else ("PM" if record.leave_period == 'pm' else record.leave_period.upper())
                    record.status_display = f"On Leave: {type_name} ({period_str})"
                else:
                    record.status_display = f"On Leave: {type_name}"
            else:
    ...
    def generate_daily_records(self, target_date=None):
        ...
        if vals_list:
            ...
            records._compute_leave_info()
            records._compute_status()
            records._compute_status_display()
    ...
    def action_refresh_time_off(self):
        """Force re-check of time off requests for selected attendance records (or today's records if none selected)."""
        records = self
        if not records:
            records = self.search([('date', '=', fields.Date.today())])
        ...
        return {
            ...
            'tag': 'display_notification',
            'params': {
                'title': _('Time Off Synchronized'),
                'message': _('Successfully checked and synchronized Time Off for %s attendance records.') % len(records),
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    @api.model
    def action_sync_all_time_off(self):
        """
        Scan and synchronize all active, approved, and requested Time Off records across all employees
        directly into daily attendance records from one place.
        """
        HrLeave = self.env['hr.leave'].sudo()
        AttendanceRecord = self.sudo()
        ...
        for leave in leaves:
            ...
            while curr_date <= date_to:
                ...
                if record:
                    ...
                    record._compute_worked_hours()
                    record._compute_expected_hours()
                    affected_record_ids.add(record.id)
                    has_record = True
        ...
        # Also clean up any attendance records whose leave was refused or cancelled
        outdated = AttendanceRecord.search([
            ('status', '=', 'on_leave'),
            ('leave_id', '!=', False),
            ('leave_id.state', 'not in', ('confirm', 'validate1', 'validate')),
        ])
        for rec in outdated:
            rec._compute_leave_info()
            rec._compute_status()
            rec._compute_status_display()
            affected_record_ids.add(rec.id)

        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': _('Time Off Synchronization Complete'),
                'message': _('Successfully synchronized %d Time Off request(s) across %d daily attendance record(s).') % (synced_count, len(affected_record_ids)),
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def auto_mark_absent(self, target_date=None):
        ...
        Checks and verifies Time Off before concluding any employee absent.
        """
        if not target_date:
            target_date = fields.Date.today()
        
        # Pre-check Time Off for all records on this date so approved leaves are not marked absent
        day_records = self.search([('date', '=', target_date)])
        if day_records:
            day_records._compute_leave_info()
            day_records._compute_status()
            day_records._compute_status_display()
