...
class AttendanceDashboard(models.Model):
    ...
    punctuality_rate = fields.Float(string='Punctuality Rate (%)', compute='_compute_dashboard_stats')
    total_overtime_today = fields.Float(string='Overtime Hours Today', compute='_compute_dashboard_stats')
    total_exceptions_today = fields.Integer(string='Exceptions Today', compute='_compute_dashboard_stats')
    ...
    def _compute_dashboard_stats(self):
        ...
        for dashboard in self:
            ...
            overtime_today = sum(records_today.mapped('overtime_hours'))
            
            punctuality = ((present_count - late_arrivals) / present_count * 100) if present_count > 0 else 100.0
            exceptions = absent_count + missed_punch_count + late_arrivals + early_leaves
            ...
            dashboard.punctuality_rate = round(punctuality, 1)
            dashboard.total_overtime_today = overtime_today
            dashboard.total_exceptions_today = exceptions
